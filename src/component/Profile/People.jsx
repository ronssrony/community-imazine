...
function People({data }) {
  ...
  const { data:activestatus , ispending , error}=useFetch(`https://imazineblue.onrender.com/api/activestatus/${data._id}`) 
  ...
  return (
    ...
    <div className='flex items-center gap-5'>
        <Link to={`/message/${data._id}`} className='flex items-center gap-2 max-sm:w-full  max-lg:w-1/2 w-1/6 truncate'> 
            <img className="w-12 h-12 rounded-[50%] object-cover" src={`https://imazineblue.onrender.com/images/uploads/${data.photo}`} alt="" />
        <div>
